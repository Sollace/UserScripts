...
if (getIsLoggedIn() && $('.module_container.module_locked .user-links a').length > 0) (function() {
    ...
    function popularity(g, l, t) {
        var story_views = parseInt($('.module_container.module_locked .avatar ~ .fa-bar-chart-o + b').html());
        ...
        var fav_count = parseInt(_count.next().next().find('.number').html());

        var result = (t > 0 ? (g - l * 100) / t : 100) + t + (blog_count * ((story_views / story_count > 0 ? story_count : 0) * fav_count / 12));
        result /= 100;
