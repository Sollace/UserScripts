...
if (!window['InteractivePony']) return;
...
function run() {
  ...
  Ponies.forEach(a => PonyNamesRegister[a.Name.toLowerCase()] = a);
  
  var allowChapterPonies = getAllowChapterPonies();

  const chapterPony = allowChapterPonies ? pickChapterPony() : null;

  var GlobalInteractivePony = null;
...
${spriteUiElement('', '', '', '')}`;
  ...
  function pickChapterPony() {
    const tagMap = {
      'princess twilight': 'twilight sparkle',
      'pinkamena': 'pinkie pie',
      'flutterbat': 'fluttershy',
      'vinyl scratch': 'dj pon-3',
      'tempest (fizzlepop) Shadow': 'tempest shadow',
      'lyra heartstrings': 'lyra',
      'changeling': 'changelings',
      'sea breeze': 'seabreeze',
      'filly rarity': 'rarity',
      'filly dash': 'rainbow dash',
      'princess flurry heart': 'flurry heart',
      'fluffle puff': 'oc'
    };
    const chapterPonies = getChapterPonies().filter(name => {
      return PonyNamesRegister[name] || (tagMap[name] && PonyNamesRegister[tagMap[name]]);
    });
    console.log('Found ponies! ' + chapterPonies);

    if (!chapterPonies.length) return null;
    const name = pickOne(chapterPonies);
    
    const normal = PonyNamesRegister[name];
    if (normal && tagMap[name] && Math.random() > 0.5) {
      return PonyNamesRegister[tagMap[name]].Id;
    }
    return normal.Id;
  }

  function getChapterPonies() {
    if (!document.querySelector('#chapter-body')) return [];
    return [].map.call(document.querySelectorAll('.tag-character'), a => {
      console.log('Found tag! ' + a);
